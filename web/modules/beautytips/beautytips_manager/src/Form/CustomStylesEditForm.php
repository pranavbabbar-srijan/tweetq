    ...
    foreach ($integer_fields as $name => $type) {
      ...
      if ($value) {
        if (!ctype_digit($value)) {
          ...
          $form_state->setErrorByName($error_element, t('You need to enter an integer value for <em>@name</em>', array('@name' => $name)));
        }
      }
    }
    ...
    foreach ($pixel_fields as $name => $type) {
      ...
      if ($value) {
        ...
        if (!is_numeric($value) || (!$value && $value != 0) || !in_array($unit, array('px', 'em'))) {
          $error_element = $type == 'css' ? 'custom_styles][css-styles][' . $name : 'custom_styles][' . $name;
          $form_state->setErrorByName($error_element, t('You need to enter a numeric value for <em>@name</em>, followed by <em>px</em> or <em>em</em>', array('@name' => $name)));
        }
      }
    }
  }
